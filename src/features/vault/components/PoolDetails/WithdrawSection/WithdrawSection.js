...
import { useFetchWithdraw} from 'features/vault/redux/fetchWithdraw';
import { useFetchBalances} from 'features/vault/redux/fetchBalances';
import { useFetchApproval} from 'features/vault/redux/fetchApproval';
...
const WithdrawSection = ({ pool, index, sharesBalance }) => {
  ...
  const withdrawOutputs = useMemo(() => {
    ...
    if (pool.zap && !pool.zap.disabled && pool.assets.length > 1) {
      const pairTokens = pool.zap.tokens
        .filter(t => !t.wrappedSymbol)
        .filter(t => (t.isVault ? t.id !== 'mlnl-mlnl' : true))
        .filter(t => (t.isVault ? t.id !== 'shade' : true))
        .filter(t => (t.isVault ? t.address !== pool.earnedTokenAddress : true));
      if (pairTokens.length) {
        outputs.push(...pairTokens);
      }
    }
  ...
  const handleOutputChange = event => {
    ...
    const isETH = isSwap && !!swapOutput.wrappedSymbol;
    const isVault = isSwap && !!swapOutput.isVault;
    const toRouter = isSwap && swapOutput.router;
    const allowance = new BigNumber(tokens[pool.earnedToken].allowance[spender]);
    console.log(
      'spender: ',
      spender,
      ' allowance: ',
      tokens[pool.earnedToken].allowance,
      allowance.isZero()
    );
  ...
  const handleApproval = () => {
    fetchApproval({
      ...
      tokenSymbol: withdrawSettings.isZap ? withdrawSettings.swapOutput.address : pool.earnedToken,
    })
    ...
    if (withdrawSettings.isZap) {
      if (withdrawSettings.isSwap) {
        const zapWithdrawArgs = {
          ...
          isVault: withdrawSettings.isVault,
          isETH: withdrawSettings.isETH,
          toRouter: withdrawSettings.toRouter,
          router: pool.zap.router,
    ...
    >
      ...
      <FormControl fullWidth variant="outlined">
        <CustomOutlinedInput
          ...
          //             {output.isVault ? output.name : output.symbol}
          //           </MenuItem>
          //         ))}
          //       </Select>
          //     </FormControl>
