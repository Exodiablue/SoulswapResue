...
import { useFetchBalances} from '../../../../vault/redux/fetchBalances';
import { useFetchZapDeposit} from '../../../../vault/redux/fetchZapDeposit';
import { useFetchApproval} from '../../../../vault/redux/fetchApproval';
import { useFetchZapEstimate} from '../../../../vault/redux/fetchZapEstimate';


import { useFetchDeposit} from '../../../../vault/redux/fetchDeposit';
...
const DepositSection = ({ pool }) => {
  ...
  const [depositSettings, setDepositSettings] = useState({
    ...
    earningToken: pool.tokenAddress,
    ammRouter: pool.zap?.router ?? '',
    isLP: pool.assets.length === 2,
  ...
  const handleTokenChange = event => {
    ...
    const isETH = !!token.wrappedSymbol;
    const allowance = new BigNumber(tokens[token.symbol].allowance[spender]);
    console.log('Token and isETH ', token, isETH);
    ...
    setDepositSettings(prevState => ({
      ...
      isNeedApproval: !isETH && allowance.isZero(),
    }));
  };
  ...
  const handleSliderChangeCommitted = (_, sliderInt) => {
    ...
    setDepositSettings(prevState => ({
      ...
      isNeedApproval: !depositSettings.token.wrappedSymbol && allowance.isZero(),
    }));
  };
  ...
  const depositAssets = deposit => {
    ...
    if (deposit.isZap) {
      // Zap deposit
      const swapAmountOut = pool.zapEstimate?.swapAmountOut ?? 0;
      ...
      const zapDepositArgs = {
        ...
        earningToken: deposit.earningToken,
  ...
  const swapTokenOut = !!t.address
    ? depositSettings.isZap
      ? eligibleTokens.find(
  ...
  const vaultFee = 'Deposit fee: ' + pool.depositFee + ', ' + 'Withdrawal fee: ' + pool.withdrawalFee;
  return (
    <Grid
      item
      xs={12}
    ...
    >
      <div className={classes.showDetailLeft}>
        ...
        <a onClick={handleMax} className={classes.balanceMax}>
          {tokenBalance(depositSettings.token.symbol)
            ...
            .toFormat()}{' '}
          {depositSettings.token.isVault
            ? `${depositSettings.token.name} Shares`
                ...
                >
                  {eligibleTokens.map((token, i) => (
                    <MenuItem key={i} value={i}>
                      {token.isVault ? token.name : token.symbol}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
      ...
      />
      {depositSettings.isZap && <div>{pool.msg}</div>}
      ...
      <p className={classes.note}>
        {pool.id === 'spiritxginspirit' ?  'You will receive GinSPIRIT token as a receipt for your deposited SPIRIT assets.'  :  pool.id === 'lqdrgxlqdr' ?  'You will receive gxLQDR token as a receipt for your deposited LQDR assets.' : t('Vault-DepositTokensNote', { mooToken: pool.earnedToken, assetToken: pool.token })  }
      </p>
    </Grid>
  );
};
