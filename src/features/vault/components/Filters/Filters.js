...
const Filters = ({
  ...
  const handlePlatformChange = useCallback((_event, option) => setPlatform(option.value), [setPlatform]);
  ...
  useEffect(() => {
    if ((!platform || !allPlatformOptions.find(option => option.value === platform)) && platform !== 'All') {
      setPlatform('All');
    }
  ...
  return (
    <Grid container spacing={2} className={classes.container}>
      ...
      <FormControl className={classes.selectorContainer}>
          <Autocomplete
            value={allPlatformOptions.find(option => option.value === platform)}
